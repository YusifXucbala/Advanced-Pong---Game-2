...
@onready var basketball_ball_timer: Timer = $Basketball_BallTimer
...
func _ready() -> void:
	...
	if not basketball_ball_timer.timeout.is_connected(_on_basketball_ball_timer_timeout):
		basketball_ball_timer.timeout.connect(_on_basketball_ball_timer_timeout)
...
func _on_spawn_timer_timeout() -> void:
	...
	selected_instance.add_to_group("Items") # Added to group for easy cleanup
	selected_instance.position = Vector2(random_x, random_y)
	
	selected_instance.z_index = 5
...
func _on_resume_pressed() -> void:
	paus = false
	pausing.visible = false
	$Basketball_Ball.speed = speed_of_ball
...
func reset_game_state() -> void:
	...
	# Clear leftover items from screen
	get_tree().call_group("Items", "queue_free")
	
	if speed_of_ball > 0:
		$Basketball_Ball.speed = speed_of_ball
	$Basketball_Ball.new_ball()

# --- GOAL DETECTORS ---

func _on_left_wall_body_entered(body: Node2D) -> void:
	if body.name == "Basketball_Ball":
		score[1] += 1
...
func _on_basketball_ball_timer_timeout() -> void:
	$Basketball_Ball.new_ball()
