...
@onready var bpaddle_1_color: ColorRect = $Bpaddle1_Color


# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	win_height = panel.get_size().y
	p_height = bpaddle_1_color.get_size().y
...
func freeze(duration: float = 2.0) -> void:
	...
	bpaddle_1_color.color = Color("26F7FD")
	
	await get_tree().create_timer(duration).timeout
	
	
	bpaddle_1_color.color = Color("943B00")
	is_frozen = false
