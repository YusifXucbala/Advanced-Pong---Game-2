...
@onready var ball_timer: Timer = $Voleyball_Ball_Timer
...
func _ready() -> void:
	...
	if not ball_timer.timeout.is_connected(_on_voleyball_ball_timer_timeout):
		ball_timer.timeout.connect(_on_voleyball_ball_timer_timeout)
...
func _on_pause_pressed() -> void:
	...
	if $Voleyball_Ball.speed > 0:
		speed_of_ball = $Voleyball_Ball.speed
	$Voleyball_Ball.speed = 0
...
func _on_left_wall_body_entered(body: Node2D) -> void:
	...
	if body.name == "Voleyball_Ball":
		...
		if score[1] >= 10:
			...
			$Voleyball_Ball_Timer.start() # Fixed typo in node path
...
func trigger_win(is_ai: bool, ai_text: String, p2_text: String) -> void:
	...
	if $Voleyball_Ball.speed > 0:
		speed_of_ball = $Voleyball_Ball.speed
	$Voleyball_Ball.speed = 0 # Fixed typo ($Football_Ball)
...
func _on_voleyball_ball_timer_timeout() -> void:
	$Voleyball_Ball.new_ball()
