...
@onready var voleyball_ball_sound: AudioStreamPlayer2D = $Voleyball_Ball_Sound
@onready var voleyball_main_game: Node2D = $".."
...
func new_ball():
	while voleyball_main_game.paus:
...
func _physics_process(delta: float) -> void:
	...
	if collision:
		...
		if collider == $"../Vpaddle1" or collider == $"../Vpaddle2":
			deactivate_speed_boost()
			make_visible()
...
func make_invisible():
	if not made_invisible:
		...
		$Voleyball_Ball_Color.modulate.a = 0
		
func make_visible():
	if made_invisible:
		made_invisible = false
		
		$Voleyball_Ball_Color.modulate.a = 1.0
